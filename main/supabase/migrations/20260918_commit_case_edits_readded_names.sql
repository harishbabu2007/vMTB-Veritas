-- Follow-up to 20260918_commit_case_edits_carry_uploads.sql: a document name
-- that was removed and later re-added (same file name) has both a removed and
-- a live case_documents row. Carrying unfinished uploads into a superseding
-- run must treat such a name as live — it only drops uploads that are
-- removed now. Same function body as the updated definition in
