-- Follow-up to 20260918_case_pipeline_runs.sql: when a save supersedes a run
-- whose uploaded documents haven't been converted and anonymized yet, the new
-- run takes those uploads over (minus any removed in the same save), so it
-- converts them and waits for them before summarizing. Previously the newer
-- run summarized immediately without them, and nothing re-summarized once
-- they were ready. Same function body as the updated definition in
